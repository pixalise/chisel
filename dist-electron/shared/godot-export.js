...
function tableFileStem(table) {
    const stem = (0, lodash_1.snakeCase)(table.name || table.id);
...
function assetExportRecord(asset) {
    ...
    const record = {
        category: (0, lodash_1.snakeCase)(asset.category),
        extension: asset.extension,
        height: asset.height,
        name: (0, lodash_1.snakeCase)(asset.name),
        path: `res://${exportPath}`,
        width: asset.width
    };
    if (isPackedTerrainTextureAsset(asset)) {
...
function renderLocalizationModule(localization, assets) {
    const iconSlugsByKey = localization.keys.map((key) => (0, localization_1.localizationIconSlugsForKey)(key, localization.defaultLocale));
    const placeholdersByKey = localization.keys.map((key) => (0, localization_1.localizationPlaceholdersForKey)(key, localization.defaultLocale));
...
const PLACEHOLDER_TYPES := ${gdValue(placeholdersByKey.map((placeholders) => placeholders.map((placeholder) => (0, localization_1.placeholderSyntaxType)(placeholder.type))))}
...
function localizationIconsDictionary(assets) {
    const iconAssets = assets.filter((asset) => asset.category === types_1.AssetCategoryEnum.uiIcon);
...
function localizationTree(keys, defaultLocale) {
    const root = {
        children: new Map(),
    ...
    for (const key of keys) {
        ...
        node.methods.push({
            key,
            name: methodName,
            placeholders: (0, localization_1.localizationPlaceholdersForKey)(key, defaultLocale)
        });
    }
    return root;
}
...
function translationMethod(method) {
...
function gdscriptPlaceholderType(placeholder) {
    if (placeholder.type === localization_1.TranslationPlaceholderType.integer) {
        return "int";
    }
    if (placeholder.type === localization_1.TranslationPlaceholderType.number) {
        return "float";
    }
    return "String";
}
function gdscriptPlaceholderDefault(placeholder) {
    if (placeholder.type === localization_1.TranslationPlaceholderType.integer) {
        return " = -1";
    }
    if (placeholder.type === localization_1.TranslationPlaceholderType.number) {
...
function renderInputExport(tables) {
    ...
    const actionNames = stringNameArray(inputTable.table.rows.map((row) => (0, lodash_1.snakeCase)(row.slug)));
    return [
        {
