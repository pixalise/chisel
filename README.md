...
Terrain authoring is fully contained in Chisel. Import PNG spritesheets into the Asset Library as **Tileset** assets, set their square tile sizes, then use **Terrain → WFC Samples** to paint rectangular examples with a required base layer and optional transparent overlay layers. Chisel compiles selectable overlapping 2×2, 3×3, or 4×4 patterns, supports deliberately periodic input, reports per-sample pattern viability, and generates seeded candidates for approval. Only approved frozen patches and compact tileset metadata appear in Data Tables and game exports; WFC source samples remain editor-only. No external map editor or intermediate map format is required.
