...
import { translationKey, type LocalizationDocument } from "./localization";
...
function renderLocalizationModule(localization: LocalizationDocument): GodotExportFile {
  const keyLines = localization.translations.map(
    (entry) => `\t${constantCase(translationKey(entry))} = ${gdString(translationKey(entry))}`
  );
  const keys = localization.translations.map((entry) => `\t${gdString(translationKey(entry))}`);
  return {
    content: `# Generated by Chisel. Do not edit.\nclass_name ChiselLocalization\nextends RefCounted\n\nenum Key {\n${keyLines.join(",\n")}\n}\n\nconst LOCALES := ${gdValue(localization.activeLocales)}\nconst KEYS := [\n${keys.join(",\n")}\n]\nconst CSV_PATH := "res://game_data/localization/translations.csv"\n`,
    path: `${GAME_DATA_EXPORT_ROOT}/localization.gd`
  };
}
